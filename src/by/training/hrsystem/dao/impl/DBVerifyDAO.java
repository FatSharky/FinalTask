...
public class DBVerifyDAO implements VerifyDAO {
	...
	private static final String SQL_VERIFY_RESUME = "UPDATE verify SET pass='pass' WHERE id_verify=?;";
	private static final String SQL_SELECT_VERIFY_BY_ID_VACANCY = "SELECT v.id_verify, v.pass, v.id_vacancy, r.id_resume, r.name, r.publish_date, r.email FROM verify as v, resume as r "
			+ "WHERE v.id_vacancy=? and r.id_resume=v.id_resume; ";
	private static final String SQL_SELECT_VERIFY_BY_ID = "SELECT verify.id_verify, user.email, user.surname, user.name, user.secondname, user.contact_phone, user.skype, vacancy.name, vacancy.publish_date "
	...
	private static final String SQL_SELECT_PASS_VERIFY_BY_ID_VACANCY = "SELECT verify.id_verify, user.email, user.surname, user.name, user.secondname, user.contact_phone, user.skype "
	...
	@Override
	public void verifyResume(int idVerify) throws DAOException {
		logger.debug("DBVerifyDAO.verifyResume() - idverify = {}", idVerify);
		Connection conn = null;
		PreparedStatement ps = null;
		ConnectionPool pool = null;
		try {
			pool = ConnectionPool.getInstance();
			conn = pool.takeConnection();
			ps = conn.prepareStatement(SQL_VERIFY_RESUME);
